...
def get_optimizer(cfg, net):
    ...
    if optimizer_mode == "adam":
        optimizer = torch.optim.Adam(
            net.parameters(), **(cfg.model[cfg.model.type][optimizer_mode])
        )
    elif optimizer_mode == "SGD":
        optimizer = torch.optim.SGD(
            net.parameters(), **(cfg.model[cfg.model.type][optimizer_mode])
        )
    elif optimizer_mode == "SAM":
        print("sam")
        print(cfg.model[cfg.model.type][optimizer_mode])
        optimizer = SAMSGD(
            net.parameters(), **(cfg.model[cfg.model.type][optimizer_mode])
            )
    else:
        raise Exception("%s optimizer not supported" % optimizer_mode)
    print(f"optimizer parameter: {cfg.model[cfg.model.type][optimizer_mode]}")
    return optimizer